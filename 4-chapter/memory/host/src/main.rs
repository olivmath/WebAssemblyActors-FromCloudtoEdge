...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let mut f = File::open("../guest/target/wasm32-unknown-unknown/release/guest.wasm")?;
    let mut bytes = vec![];
    ...
    let getptr = module.find_function::<(), i32>("get_buffer_pointer")?;
    let ptr = getptr.call()?;
    println!("{}", ptr);

    // Write the name to memory
    let name = b"Lucas Oliveira";
